...
def _parse_rate_from_commodity(payload: dict[str, Any], measure_type: str) -> float:
    """Extract measure rate from HMRC commodity payload."""
    included = payload.get("included") or []
    if not isinstance(included, list):
        raise VanikAPIError(
            code="parse_error",
            message="UK payload missing included array",
            source=_SOURCE,
        )
...
def get_uk_mfn_rate(commodity_code_10: str, *, client: httpx.Client | None = None) -> TariffResult:
    """Fetch UK MFN rate for a 10-digit declarable commodity code."""
    if not commodity_code_10.isdigit() or len(commodity_code_10) != 10:
        raise VanikAPIError(
            code="invalid_hs_code",
            message="UK destination requires a 10-digit commodity code",
            source=_SOURCE,
    ...
    try:
        response = http_client.get(
            f"{settings.uk_base_url.rstrip('/')}/commodities/{commodity_code_10}",
            headers={"Accept": settings.hmrc_accept_header},
        )
    except httpx.HTTPError as exc:
        raise VanikAPIError(
            code="upstream_unreachable",
            message="Failed to connect to UK Trade Tariff API",
            source=_SOURCE,
    ...
    if response.status_code >= 400:
        raise VanikAPIError(
            code="upstream_error",
            message="UK Trade Tariff API returned an error",
            source=_SOURCE,
