...
def test_parse_upload_csv_lane_b_reference() -> None:
    csv = (
        "product,origin,destination,po_number,supplier\n"
        "widgets,IN,GB,PO-99,VendorCo\n"
    )
    rows = parse_upload_csv(csv)
    assert len(rows) == 1
    assert rows[0]["reference"]["po_number"] == "PO-99"
    assert rows[0]["reference"]["supplier"] == "VendorCo"
